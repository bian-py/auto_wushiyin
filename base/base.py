...
class Base:
    ...
    def base_get_question_answer(self, loc_question, loc_answer):
        el = self.base_find_element(loc_question)
        question = el.text
        # print(el,question)
        log.info(f"正在获取问题，问题是{question}")
        ss = question[::-1][1]
        result = str(int(ss) + 1)
        log.info(f"答案是第{result}个")
        ans_str = loc_answer[1]
        s = ans_str[:len(ans_str) - 1:]
        loc_ans_tar = loc_answer[0], s + result
        log.info("选中答案")
        self.base_click_element(loc_ans_tar)
